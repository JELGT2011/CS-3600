...
def consistent(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            affected = constraint.otherVariable(var)
            if not constraint.isSatisfied(value, assignment.assignedValues[affected]):
                return False
    return True
...
def minimumRemainingValuesHeuristic(assignment, csp):
    ...
    for var in assignment.assignedValues:
        if not assignment.isAssigned(var):
            if len(domains[var]) < len(domains[next_variable]):
                next_variable = var
            elif len(domains[var]) == len(domains[next_variable]):

                variable_constraints = 0
                next_variable_constraints = 0

                for constraint in csp.binaryConstraints:
                    if constraint.affects(var):
                        variable_constraints += 1
                    if constraint.affects(next_variable):
                        next_variable_constraints += 1

                if variable_constraints > next_variable_constraints:
                    next_variable = var
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
    ...
    def compare_constraining_values(x, y):
        ...
        for constraint in csp.binaryConstraints:
            if constraint.affects(var):
                xt = constraint.otherVariable(var)
                if x in assignment.varDomains[xt]:
                    x_eliminates += 1

                yt = constraint.otherVariable(var)
                if y in assignment.varDomains[yt]:
                    y_eliminates += 1

        return x_eliminates - y_eliminates

    values = list(assignment.varDomains[var])

    return sorted(values, key=functools.cmp_to_key(compare_constraining_values))
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
    ...
    for value in orderValuesMethod(assignment, csp, variable):
        ...
        assignment.assignedValues[variable] = value
        inferences = inferenceMethod(assignment, csp, variable, value)
...
def revise(assignment, csp, var1, var2, constraint):
    ...
    for d_var2 in assignment.varDomains[var2]:

        satisfiable = False
        for d_var1 in assignment.varDomains[var1]:
            if constraint.isSatisfied(d_var1, d_var2):
                satisfiable = True

        if not satisfiable:
            inferences.add((var2, d_var2))
...
def maintainArcConsistency(assignment, csp, var, value):
    ...
    for constraint in csp.binaryConstraints:
        if constraint.affects(var):
            arcs.put((var, constraint.otherVariable(var), constraint))

    while not arcs.empty():

        arc = arcs.get()
        revisions = revise(assignment, csp, arc[0], arc[1], arc[2])

        if revisions is not None:
            for inference in revisions:
                inferences.add(inference)
                for constraint in csp.binaryConstraints:
                    if constraint.affects(arc[1]) and not constraint.affects(arc[0]):
                        arcs.put((arc[1], constraint.otherVariable(arc[1]), constraint))
        else:
            for inference in inferences:
                assignment.varDomains[inference[0]].add(inference[1])
            return None
...
def AC3(assignment, csp):
    ...
    for variable in assignment.assignedValues:
        inferences = maintainArcConsistency(assignment, csp, variable, None)
        if not inferences:
            return None
